...
type Props = {
  value: WhatsappIntegrationForm;
  status: Status;
  onChange: (next: WhatsappIntegrationForm) => void;
};

export const WhatsappIntegrationCard: React.FC<Props> = ({ value, status, onChange }) => {
  const setField = (path: keyof WhatsappIntegrationForm['api'] | 'curlTemplate', nextValue: string) => {
    if (value.type === 'api') {
      onChange({
        ...value,
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-lg font-bold text-slate-900">WhatsApp Notifications</h3>
          <p className="text-xs text-slate-500">Configure WhatsApp API or CURL template.</p>
        </div>
        <span
